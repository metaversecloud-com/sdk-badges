...
**Secrets:** no plaintext committed. `badges0-sealedsecret.yaml` holds only ciphertext
(sealed strict-scope for namespace sdk-apps-dev); the controller unseals it into
`Secret badges0-secrets`, consumed via `envFrom`. Non-secret env (incl. public
`INTERACTIVE_KEY`) lives in `badges0-config`.

## Environment

| Env | Service | Namespace | Host | Health |
| --- | ------- | --------- | ---- | ------ |
| `dev` | `badges0` | `sdk-apps-dev` | badges0-dev-topia.topia-rtsdk.com | `/api/system/health` |
